...
def fix_cell_14(source):
    ...
    new_source.append('        return float(r.stdout.strip())\n')
    ...
    new_source.append('    while r < 0.5:\n')
    new_source.append('        filters.append("atempo=0.5"); r /= 0.5\n')
    new_source.append('    while r > 2.0:\n')
    new_source.append('        filters.append("atempo=2.0"); r /= 2.0\n')
    ...
    new_source.append('    get_ipython().system(f"ffmpeg -y -i {TTS_RAW} -filter:a \'{atempo}\' -ar 44100 {TTS_ADJ} -loglevel warning")\n')
    new_source.append('    print(f"✓ Audio adjusted to {clip_dur:.2f}s")\n')
...
def fix_cell_16(source):
    ...
    new_source.append('PROCESSOR = "cuda" if torch.cuda.is_available() else "cpu"\n')
    new_source.append('get_ipython().system(f"python GFPGAN/inference_gfpgan.py --ext png -i {FRAMES_DIR} -o {ENHANCED_DIR} -v 1.4 -s 1 --bg_upsampler None")\n')
    new_source.append('\n')
