...
def fix_cell_14(source):
    ...
    new_source.append('import subprocess, os, numpy as np\n')
    new_source.append('\n')
    ...
    new_source.append('    # Stage 6a: Speed match\n')
    new_source.append('    get_ipython().system(f"ffmpeg -y -i {TTS_RAW} -filter:a \'{atempo}\' -ar 16000 workspace/tts_speed_matched.wav -loglevel warning")\n')
    new_source.append('\n')
    new_source.append('    # Stage 6b: Auto-Align (Advanced Lip Sync Match)\n')
    new_source.append('    print("Checking audio-video synchronization...")\n')
    new_source.append('    try:\n')
    new_source.append('        import librosa\n')
    new_source.append('        from utils import check_sync_offset\n')
    new_source.append('        # Extract original audio from CLIP to compare\n')
    new_source.append('        get_ipython().system(f"ffmpeg -y -i {CLIP} -vn -ar 16000 workspace/orig_audio.wav -loglevel warning")\n')
    new_source.append('        offset_sec, confidence = check_sync_offset("workspace/orig_audio.wav", "workspace/tts_speed_matched.wav")\n')
    new_source.append('        print(f"Detected sync offset: {offset_sec*1000:.1f}ms (confidence: {confidence:.3f})")\n')
    new_source.append('\n')
    new_source.append('        if abs(offset_sec) > 0.01: # Shift if > 10ms\n')
    new_source.append('            if offset_sec > 0:\n')
    new_source.append('                # Dubbed is delayed -> Trim start\n')
    new_source.append('                get_ipython().system(f"ffmpeg -y -i workspace/tts_speed_matched.wav -af \'atrim=start={offset_sec},asetpts=PTS-STARTPTS\' {TTS_ADJ} -loglevel warning")\n')
    new_source.append('            else:\n')
    new_source.append('                # Dubbed is early -> Delay it\n')
    new_source.append('                delay_ms = int(abs(offset_sec) * 1000)\n')
    new_source.append('                get_ipython().system(f"ffmpeg -y -i workspace/tts_speed_matched.wav -af \'adelay={delay_ms}|{delay_ms}\' {TTS_ADJ} -loglevel warning")\n')
    new_source.append('            print(f"✓ Audio aligned by {abs(offset_sec)*1000:.1f}ms")\n')
    new_source.append('        else:\n')
    new_source.append('            os.replace("workspace/tts_speed_matched.wav", TTS_ADJ)\n')
    new_source.append('    except Exception as e:\n')
    new_source.append('        print(f"⚠️ Auto-alignment failed ({e}). Using speed-matched version.")\n')
    new_source.append('        os.replace("workspace/tts_speed_matched.wav", TTS_ADJ)\n')
    new_source.append('else:\n')
...
for cell in nb['cells']:
    ...
    if '# ── Cell 15: Stage 7 — VideoReTalking Lip Sync' in source[0]:
        ...
        new_source.append('     "--pads", "0", "20", "0", "0",\n')
        new_source.append('     "--img_size", "384",\n')
